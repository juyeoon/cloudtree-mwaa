...
def execute_athena_query(database_name, file_name):
    ...
    """
    # S3
    query_key = f"{ATHENA_QUERY_PREFIX}/{file_name}.sql"
    s3 = boto3.client("s3")
    response = s3.get_object(Bucket=ATHENA_QUERY_BUCKET, Key=query_key)
    ...
    athena.start_query_execution(
        ...
        ResultConfiguration={"OutputLocation": f"s3://{ATHENA_QUERY_OUTPUT_BUCKET}/{ATHENA_QUERY_OUTPUT_PREFIX}/"},
    )
...
def msck_repair_table(database_name, table_name, retry=5, retry_delay=5):
    ...
    return AthenaOperator(
        ...
        output_location=f"s3://{ATHENA_QUERY_OUTPUT_BUCKET}/{ATHENA_QUERY_OUTPUT_PREFIX}/",
        retries=retry,
        retry_delay=timedelta(seconds=retry_delay),
        on_retry_callback=send_slack_message_fail,
        on_failure_callback=send_slack_message_fail,
