...
with DAG(
    ...
    raw_bus_stop_loc_table = "bus_stop_loc_raw"
    raw_bus_stop_loc_database_name = "cloudtree_raw_db"
    raw_bus_stop_loc_sql_file_name = f"create_table_{raw_bus_stop_loc_table}"
    raw_bus_stop_loc_fetch_s3_task_id = f"fetch_sql_{raw_bus_stop_loc_sql_file_name}"
    raw_bus_stop_loc_table_ddl = fetch_query_s3(
        raw_bus_stop_loc_fetch_s3_task_id, f"create_table_{raw_bus_stop_loc_table}"
    )
    raw_bus_stop_loc_create_table_by_ddl = create_table(
        raw_bus_stop_loc_fetch_s3_task_id, raw_bus_stop_loc_database_name
    )

    trans_bus_stop_loc_table = "bus_stop_loc"
    trans_bus_stop_loc_database_name = "cloudtree_transformed_db"
    trans_bus_stop_loc_sql_file_name = f"create_table_{trans_bus_stop_loc_table}"
    trans_bus_stop_loc_fetch_s3_task_id = f"fetch_sql_{trans_bus_stop_loc_sql_file_name}"
    trans_bus_stop_loc_table_ddl = fetch_query_s3(
        trans_bus_stop_loc_fetch_s3_task_id, f"create_table_{trans_bus_stop_loc_table}"
    )
    trans_bus_stop_loc_create_table_by_ddl = create_table(
        trans_bus_stop_loc_fetch_s3_task_id, trans_bus_stop_loc_database_name
    )

    invoke_bus_stop_loc_API = invoke_lambda("getBusStopLocAPI", {"key": seoul_key})
    msck_bus_stop_loc_raw = update_data_catalog("cloudtree_raw_db", "bus_stop_loc_raw")
    run_glue_bus_stop_loc = run_glue_job("bus_stop_loc_job")
    msck_bus_stop_loc_trans = update_data_catalog("cloudtree_transformed_db", "bus_stop_loc")

    # task 의존성 설정

    (
        raw_bus_stop_loc_table_ddl
        >> raw_bus_stop_loc_create_table_by_ddl
        >> trans_bus_stop_loc_table_ddl
        >> trans_bus_stop_loc_create_table_by_ddl
        >> invoke_bus_stop_loc_API
        >> msck_bus_stop_loc_raw
        >> run_glue_bus_stop_loc
        >> msck_bus_stop_loc_trans
    )
